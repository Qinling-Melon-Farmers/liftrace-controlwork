...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  node_.param("sdf_map/obstacles_inflation_up", mp_.obstacles_inflation_up_, -1.0);
  node_.param("sdf_map/obstacles_inflation_down", mp_.obstacles_inflation_down_, -1.0);
...
void SDFMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& img) {
  ...
  int inf_step = ceil(mp_.obstacles_inflation_ / mp_.resolution_);
  int inf_step_z_up = ceil(mp_.obstacles_inflation_up_ / mp_.resolution_);
  int inf_step_z_down = ceil(mp_.obstacles_inflation_down_ / mp_.resolution_);
