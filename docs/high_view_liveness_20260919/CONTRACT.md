...
- 2026-09-20补充：实验入口可启用traj_server显式保持恢复。SERVER消息必须与当前`goal_seq + traj_id + traj_start`一致且新鲜，`tracking_hold`持续0.25s后请求重规划；它与物理无进展共享同一目标级预算，耗尽后输出终端失败。新`initial_plan_timeout`统一监督所有从未产生有效轨迹的规划动作。
